...
import { TurboQuantBetaCodebook } from './codebook.js';
import { quantizeValues, concatValueQuantized } from './value_quant.js';
// === Donor constants ===
...
function concatKeyQuantized(a, b) {
...
function precomputeScaledCentroids(centroids, queryQ8) {
...
export class CompressedKVCache {
    ...
    sparseThreshold;
    useQ8Query;
    outlierAwareRotation;
    sinkTokens;
    rotationEngine;
    codebook;
    quantizer;
    centroidValues;
    channelOrder = null; // RotateKV: outlier-sorted channel indices
    seqLen = 0;
    keyQuantized = null;
    valueQuantized = null;
    keyBuffer = null;
     ...
     */
    computeChannelOrder(keys, numTokens) {
    ...
    reorderChannels(vec, order) {
        const out = new Float32Array(vec.length);
        for (let i = 0; i < order.length; i++)
            out[i] = vec[order[i]];
        return out;
    }
    inverseReorderChannels(vec, order) {
        const out = new Float32Array(vec.length);
        for (let i = 0; i < order.length; i++)
            out[order[i]] = vec[i];
        return out;
    }
    quantizeKeys(keys, numTokens) {
        const d = this.headDim;
        ...
        if (this.useQ8Query) {
            ...
            scoreFn = (t) => {
                ...
                for (let i = 0; i < d; i++) {
                    ...
                    if (bitPos + kq.bits > 8)
                        idx |= ((kq.indices[bytePos + 1] << (8 - bitPos)) & kMask);
    ...
    dequantizeKeys(kq) {
        ...
        for (let t = 0; t < kq.numTokens; t++) {
            ...
            for (let i = 0; i < d; i++) {
                ...
                if (bitPos + kq.bits > 8)
                    idx |= ((kq.indices[bytePos + 1] << (8 - bitPos)) & mask);
            ...
            for (let i = 0; i < d; i++)
                result[t * d + i] = unrotated[i] * norm;
        }
        return result;
    }
    prefill(keys, values, numTokens) {
        const d = this.headDim;
        ...
        this.valueQuantized = quantizeValues(values.subarray(0, nQuant * d), nQuant, d, this.valueBits, this.valueGroupSize);
    ...
    append(key, value) {
        ...
        if (this.bufferTokenCount > this.bufferSize)
            this.flushBuffer();
    }
    flushBuffer() {
        const d = this.headDim;
        ...
        const newValQ = quantizeValues(new Float32Array(this.valueBuffer.subarray(0, nFlush * d)), nFlush, d, this.valueBits, this.valueGroupSize);
    ...
    getSeqLength() { return this.seqLen; }
    getKeyQuantized() { return this.keyQuantized; }
    getValueQuantized() { return this.valueQuantized; }
    getKeyBuffer() { return this.keyBuffer; }
    getValueBuffer() { return this.valueBuffer; }
    getBufferTokenCount() { return this.bufferTokenCount; }
    getSparseStats() {
        return {
    ...
    memoryBytes() {
        ...
        if (this.keyBuffer)
            buffer += this.keyBuffer.byteLength;
        if (this.valueBuffer)
            buffer += this.valueBuffer.byteLength;
