...
export function recommendCostActions(metrics) {
    const actions = [];
...
export function buildCachePlan(blocks, target) {
    ...
    const largeStableBlocks = processedBlocks.filter(b => b.recommendedTier === 'context_pack_candidate');
    if (largeStableBlocks.length > 0) {
        recommendations.push(`Large stable documents (${largeStableBlocks.length}) should be moved to compressed context pack and retrieved via context_pack_search.`);
    }
    const warnings = [
