...
function resolveExternalStoreEntry(chunkId, fingerprint, externalStore) {
    if (!externalStore) {
        return {
            text: '',
            source: 'missing',
...
export function turboquantContextPackSearch(rawInput) {
    ...
    if (input.manifest.storageMode === 'external_store') {
        if (!input.externalStore) {
            warnings.push('Manifest storageMode=external_store but no externalStore provided; text will be empty. Pass externalStore={kind:"inline_entries",entries:[...]} to resolve.');
        }
        else {
            warnings.push('External store entries resolved inline. Fingerprint verification applied per chunk.');
        }
    }
    if (input.manifest.storageMode === 'preview_only') {
        warnings.push('Only text previews are available. For full text, rebuild with storageMode=inline_text.');
    }
    return {
