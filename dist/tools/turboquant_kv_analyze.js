 ...
 */
export function turboquantKvAnalyze(input) {
    ...
    if (input.action === 'estimate_savings') {
        ...
        const mseTable = { 1: 0.05, 2: 0.005, 3: 0.0005, 4: 0.00007, 5: 0.00001, 6: 0.000002 };
...
export const turboquantKvAnalyzeSchema = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            headDim: { type: 'number', description: 'Model head dimension' },
            numKvHeads: { type: 'number', description: 'Number of KV heads (default 8)' },
            numLayers: { type: 'number', description: 'Number of layers (default 32)' },
            seqLen: { type: 'number', description: 'Sequence length (default 4096)' },
            keyBits: { type: 'number', description: 'Key bits 1-6 (default 4)' },
            valueBits: { type: 'number', description: 'Value bits 2 or 4 (default 2)' },
            bufferSize: { type: 'number', description: 'Recent unquantized tokens (default 128)' },
            memoryBudgetMb: { type: 'number', description: 'Target memory budget in MB (for recommend_config)' },
        },
