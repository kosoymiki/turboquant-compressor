...
def run_runtime() -> dict:
    result = subprocess.run(
        ["python3", str(ROOT / "bin" / "turboquant_agent_runtime.py"), "--agent", "codex_cli", "--output", str(HOME / ".codex" / ".turboquant-runtime.json")],
        cwd=ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def run_preflight(prompt: str) -> dict | None:
    ...
    result = subprocess.run(
        [
            ...
            prompt,
            "--output",
            str(HOME / ".codex" / ".turboquant-preflight.json"),
        ],
        cwd=ROOT,
        check=True,
        capture_output=True,
...
def main() -> int:
    ...
    packet = {
        ...
        "preflight": {
            ...
            "path": str(HOME / ".codex" / ".turboquant-preflight.json"),
        },
    }
    out = HOME / ".codex" / ".turboquant-proxy.json"
