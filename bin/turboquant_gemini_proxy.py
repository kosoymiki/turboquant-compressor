...
def run_runtime() -> dict:
    result = subprocess.run(
        ["python3", str(ROOT / "bin" / "turboquant_agent_runtime.py"), "--agent", "gemini_cli", "--output", str(HOME / ".gemini" / ".turboquant-runtime.json")],
        cwd=ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def run_preflight(prompt: str | None) -> dict | None:
    ...
    result = subprocess.run(
        [
            ...
            prompt,
            "--output",
            str(HOME / ".gemini" / ".turboquant-preflight.json"),
        ],
        cwd=ROOT,
        check=True,
        capture_output=True,
...
def main() -> int:
    ...
    packet = {
        ...
        "preflight": {
            ...
            "path": str(HOME / ".gemini" / ".turboquant-preflight.json"),
        },
    }
    out = HOME / ".gemini" / ".turboquant-proxy.json"
