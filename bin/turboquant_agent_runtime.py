...
DRIVER_PACK = ROOT / "native" / "opencl" / "driver-pack"
...
def detect_llvm_root() -> Path | None:
    ...
    candidates.extend([HOME / "llvm-current", HOME / "llvm"])
...
def detect_driver_library() -> str | None:
    candidates = [
        Path(os.environ.get("TQ_DRIVER_ROOT", "")) / "layer1-compute" / "libRusticlOpenCL.so.1",
...
def build_env(agent: str) -> tuple[dict, dict]:
    ...
    policy = runtime.get("driver_policy", {})
    backend = runtime.get("backend_probe", {})
    opencl_probe = runtime.get("opencl_probe", {})
    cache_dir = HOME / ".cache" / "turboquant-rusticl"
    cache_dir.mkdir(parents=True, exist_ok=True)

    production_route = policy.get("productionRoute") or backend.get("recommendedBackend") or "typescript_cpu"
    runtime_verified = bool(policy.get("runtimeExecutionVerified"))
    gpu_ready = bool(
        policy.get("productionReady")
    ...
    env = {
        ...
        "TURBOQUANT_RUNTIME_EXECUTION_VERIFIED": "1" if runtime_verified else "0",
    ...
    if llvm_bin and llvm_bin.exists():
        env["PATH"] = ":".join([str(llvm_bin), os.environ.get("PATH", "")]).strip(":")
    ...
    env["TURBOQUANT_GPU_OFFLOAD_PREFERRED"] = "1" if gpu_ready else "0"
    env["TURBOQUANT_NUMERIC_BACKEND"] = production_route if gpu_ready else "disabled_unverified_custom_stack"
    ...
    payload = {
        ...
        "llvm_root": str(llvm_root) if llvm_root else None,
