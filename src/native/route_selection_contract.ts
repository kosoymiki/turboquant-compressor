export type LoaderNamespaceContractClass =
  | 'ready'
  | 'vendor_abi_mismatch'
  | 'dependency_chain_incomplete'
  | 'render_node_policy_gate'
  | 'namespace_or_icd_blocked'
  | 'unknown';

export type RouteSelectionContractClass =
  | 'custom_preferred_vendor_abi_blocked'
  | 'custom_ready_vendor_diagnostic_only'
  | 'vendor_only_custom_blocked'
  | 'custom_and_vendor_divergent'
  | 'unknown';

export interface LoaderNamespaceContractInputs {
  vendorProbeStatus?: number | null;
  vendorProbeStderr?: string | null;
  customProbeStatus?: number | null;
  customRunAsProbeStatus?: number | null;
  customRunAsPlatformCount?: number | null;
  customRunAsDeviceCount?: number | null;
  customLoadedLibraryPath?: string | null;
  runAsStageStatus?: number | null;
  renderNodeAccessible?: boolean | null;
  kgslAccessible?: boolean | null;
  customLinkerUsesStagedOpenCl?: boolean | null;
}

export interface RouteSelectionContractInputs {
  vendorLoader: LoaderNamespaceContractClass;
  customLoader: LoaderNamespaceContractClass;
  customRunAsPlatformCount?: number | null;
  customRunAsDeviceCount?: number | null;
  customHasExternalMemory?: boolean | null;
  customHasSvm?: boolean | null;
  vendorProbeUsable?: boolean | null;
}

export interface RouteContractVerdict<TClass extends string> {
...
export function classifyLoaderNamespaceContract(
  inputs: LoaderNamespaceContractInputs,
): RouteContractVerdict<LoaderNamespaceContractClass> {
...
export function classifyRouteSelectionContract(
  inputs: RouteSelectionContractInputs,
): RouteContractVerdict<RouteSelectionContractClass> {
  const blockers: string[] = [];
  if (inputs.vendorLoader === 'vendor_abi_mismatch') blockers.push('vendor_abi_mismatch');
  if (inputs.customLoader !== 'ready' && inputs.customLoader !== 'vendor_abi_mismatch' && inputs.customLoader !== 'render_node_policy_gate') {
