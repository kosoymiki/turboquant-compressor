export type RenderNodePolicyContractClass =
  | 'ready'
  | 'kgsl_route_operational'
  | 'render_node_policy_gate'
  | 'device_node_missing'
  | 'unknown';

export type StagedDependencyChainContractClass =
  | 'ready'
  | 'dependency_chain_incomplete'
  | 'abi_mismatch_before_opencl'
  | 'unknown';

export type SpirvCapabilityContractClass =
  | 'clean'
  | 'warning_only_generic_pointer'
  | 'warning_changes_runtime_outcome'
  | 'unknown';

export interface RenderNodePolicyContractInputs {
  renderNodeAccessible?: boolean | null;
  kgslAccessible?: boolean | null;
  renderNodeMeta?: string | null;
  customRunAsPlatformCount?: number | null;
  customRunAsDeviceCount?: number | null;
  customHasExternalMemory?: boolean | null;
  customHasSvm?: boolean | null;
}

export interface StagedDependencyChainContractInputs {
  runAsStageStatus?: number | null;
  customRunAsPlatformCount?: number | null;
  missingLibs?: string[];
  vendorAbiMismatch?: boolean | null;
}

export interface SpirvCapabilityContractInputs {
  genericPointerWarningSeen?: boolean | null;
  allowInvalidSpirvChangesProbe?: boolean | null;
  allowInvalidSpirvChangesFrontier?: boolean | null;
}

export interface KernelBlockerVerdict<TClass extends string> {
...
export function classifyRenderNodePolicyContract(
  inputs: RenderNodePolicyContractInputs,
): KernelBlockerVerdict<RenderNodePolicyContractClass> {
...
export function classifyStagedDependencyChainContract(
  inputs: StagedDependencyChainContractInputs,
): KernelBlockerVerdict<StagedDependencyChainContractClass> {
  const blockers: string[] = [];
...
export function classifySpirvCapabilityContract(
  inputs: SpirvCapabilityContractInputs,
): KernelBlockerVerdict<SpirvCapabilityContractClass> {
  const blockers: string[] = [];
