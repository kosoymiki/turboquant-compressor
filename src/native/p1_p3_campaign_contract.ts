...
export interface P1P3ClusterInputs {
  id: P1P3ClusterId;
...
export interface P1P3ClusterVerdict {
  id: P1P3ClusterId;
  priority: number;
  active: boolean;
  summary: string;
...
export interface P1P3CampaignPlan {
  queue: P1P3ClusterVerdict[];
  rationale: string;
}

const CLUSTER_ORDER: Record<P1P3ClusterId, number> = {
...
const CLUSTER_SUMMARY: Record<P1P3ClusterId, string> = {
  p1_kernel_primitives:
...
export function chooseP1P3CampaignPlan(inputs: P1P3ClusterInputs[]): P1P3CampaignPlan {
  const queue = inputs
