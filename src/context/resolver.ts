 ...
 */
export class InlineResolver implements ContextStoreResolver {
  private readonly chunks: Map<string, { text: string; fingerprint: string }>;

  constructor(chunks: Array<{ id: string; text: string; fingerprint: string }>) {
    this.chunks = new Map(chunks.map((c) => [c.id, { text: c.text, fingerprint: c.fingerprint }]));
  }

  async resolve(chunkId: string, fingerprint: string): Promise<ResolveResult> {
 ...
 */
export class PreviewResolver implements ContextStoreResolver {
  private readonly previews: Map<string, string>;

  constructor(chunks: Array<{ id: string; textPreview?: string }>) {
    this.previews = new Map(chunks.map((c) => [c.id, c.textPreview ?? '']));
  }

  async resolve(chunkId: string, _fingerprint: string): Promise<ResolveResult> {
 ...
 */
export class ExternalStoreResolver implements ContextStoreResolver {
  async resolve(chunkId: string, _fingerprint: string): Promise<ResolveResult> {
    return {
      text: '',
      source: 'external',
