...
export function validateFiniteNumber(value: number, name: string): void {
  if (!Number.isFinite(value)) {
    throw new Error(`${name} must be finite, got ${value}`);
  }
}

export function validateRectangular2D(vectors: number[][], name: string): void {
  if (vectors.length === 0) {
...
export function validateStrictBase64(value: string, name: string): void {
  if (value.length === 0) {
    throw new Error(`${name} must not be empty`);
  }
...
export function parseCompressInput(input: unknown): CompressInput {
