...
import {
  ...
  validateContextTotalBytes,
  validateContextFileBytes,
  validateContextPathLength,
  validateChunkBytes,
  validateInlineChunkBytes,
  MAX_CONTEXT_TOTAL_BYTES,
  MAX_CONTEXT_FILE_BYTES,
  MAX_INLINE_CHUNK_BYTES,
} from '../cost/limits.js';
...
function chunkTextForPack(text: string, maxBytes: number): string[] {
  const chunks: string[] = [];
  let current = '';
  ...
  if (current.trimEnd().length > 0) chunks.push(current.trimEnd());
  return chunks;
}

export function turboquantContextPackBuild(
  rawInput: unknown
  ...
  const manifest: ContextPackManifest = {
    ...
    chunks: allChunks.map((c, idx) => {
      const chunkInfo: ContextPackManifest['chunks'][0] = {
        id: `chunk_${idx}`,
        path: c.path,
        fingerprint: c.fingerprint,
        approximateTokens: c.approximateTokens,
  ...
  if (input.storageMode === 'external_store') {
    warnings.push(
      'Context text is not embedded in manifest; caller must resolve chunk_id from external store.'
    );
  }
