...
export function normalizeUsageSnapshots(
...
export function sanitizeUsageSnapshots(
  snapshots: UsageSnapshot[]
): UsageSnapshot[] {
...
export function validateUsageSnapshot(snapshot: UsageSnapshot): void {
  ...
  if (
    !Number.isFinite(snapshot.cacheCreationInputTokens) ||
    snapshot.cacheCreationInputTokens < 0
  ) {
    throw new Error(
      `Invalid cacheCreationInputTokens: ${snapshot.cacheCreationInputTokens}`
    );
  }
  if (
    !Number.isFinite(snapshot.cacheReadInputTokens) ||
    snapshot.cacheReadInputTokens < 0
  ) {
    throw new Error(
      `Invalid cacheReadInputTokens: ${snapshot.cacheReadInputTokens}`
    );
  }
}