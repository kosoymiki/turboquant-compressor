...
function createWasteSignal(
  type: WasteSignal['type'],
  severity: WasteSignal['severity'],
...
export function computeCacheEfficiency(
  ...
  options?: { invalidMode?: 'reject' | 'clamp' }
): CacheEfficiencyMetrics {
  ...
  const cacheReadToWriteRatio =
    totalCacheWrite > 0 ? totalCacheRead / totalCacheWrite : 0;
  const cacheWriteShare =
    totalInput > 0 ? totalCacheWrite / totalInput : 0;
  const cacheReadShare =
    totalInput > 0 ? totalCacheRead / totalInput : 0;
  ...
  if (totalInput >= 100000 && cacheReadToWriteRatio >= 0.5 && cacheReadToWriteRatio < 1.0) {
    wasteSignals.push(
      createWasteSignal('context_bloat', 'high', {
  ...
  if (cacheWriteShare >= 0.25 && cacheReadToWriteRatio < 0.5) {
    ...
    wasteSignals.push(
      createWasteSignal('write_heavy', severity, {
  ...
  if (cacheReadToWriteRatio < 0.5 && totalCacheWrite > 10000) {
    ...
    wasteSignals.push(
      createWasteSignal('low_cache_reuse', severity, {
  ...
  if (outputRatio >= 0.5 && totalInput < 50000) {
    ...
    wasteSignals.push(
      createWasteSignal('output_heavy', severity, {
        outputRatio,
  ...
  let estimatedWasteClass: CacheEfficiencyMetrics['estimatedWasteClass'] = 'unknown';
  ...
  if (totalCacheWrite === 0 && totalCacheRead === 0) {
    ...
    const sortedSignals = [...wasteSignals].sort(
    ...
    switch (signal.type) {
      case 'write_heavy':
        recommendations.push(
          'Cache writes dominate reads; stabilize reusable context and avoid dynamic content in early prompt prefix.'
        );
        recommendations.push(
          'Measure with ccusage before claiming savings.'
        );
        break;
      case 'context_bloat':
        recommendations.push(
          'Large context detected; consider using context_pack_build to compress local documentation.'
        );
        recommendations.push(
          'Use context_pack_search to retrieve only relevant chunks instead of dumping entire files.'
        );
        break;
      case 'low_cache_reuse':
        recommendations.push(
          'Cache reuse is low; review what content is being cached and consider stabilizing more context.'
        );
        break;
      case 'output_heavy':
        recommendations.push(
          'Output tokens are significant; consider reducing verbose tool outputs.'
        );
        break;
    }
  }
  ...
  if (metrics.wasteSignals.length === 0) {
    switch (metrics.estimatedWasteClass) {
      case 'healthy_cache_reuse':
        recommendations.push(
          'Cache efficiency looks healthy; continue monitoring with ccusage.'
        );
        break;
      default:
        recommendations.push(
          'Cache metrics unclear; ensure ccusage data is properly configured.'
        );
    }
  }
