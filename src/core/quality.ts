 ...
 */
function calculateAverageCosineSimilarity(
...
): { compressed: Uint8Array; decompressed: Float32Array[] } {
 ...
 */
export function runQualityBenchmarks(config: QualityConfig): QualityMetrics {
  ...
  for (let i = 0; i < database.length; i++) {
    ...
    for (let j = 0; j < dimensions; j++) {
      ...
      if (orig === undefined || recon === undefined) continue;
      if (Number.isNaN(orig) || Number.isNaN(recon)) continue;
...
export function runQualitySuite(
  baseConfig: Partial<QualityConfig> = {}
): { config: QualityConfig; metrics: QualityMetrics }[] {
  const results: { config: QualityConfig; metrics: QualityMetrics }[] = [];
 ...
 */
export function formatQualityTable(
  results: { config: QualityConfig; metrics: QualityMetrics }[]
): string {
  const header = '| Config | Recall@1 | Recall@10 | MSE | Cosine Sim | Compression |';
  const separator = '|--------|----------|----------|-----|------------|-------------|';

  const rows = results.map(r =>
    `| ${r.config.bitsPerValue}b ${r.config.codebookType} qjl:${r.config.useQJL} hadamard:${r.config.rotationMode === 'hadamard'} | ` +
 ...
 */
export function exportQualityJSON(
  results: { config: QualityConfig; metrics: QualityMetrics }[]
): string {
