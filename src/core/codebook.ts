...
import { sphereCoordinateBetaPdf, integrateAdaptiveSimpson, conditionalMeanBetaSphere, computeLloydMaxBetaCodebook, TurboQuantCodebook } from './beta_sphere.js';
...
export class LloydMaxCodebook {
  ...
  constructor(bitsPerValue: number = 4) {
   ...
   */
  train(data: Float32Array, maxIterations: number = 50, tolerance: number = 1e-6): Codebook {
...
export class BetaCodebook {
  private codebook: Codebook | null = null;
  private params: BetaParameters | null = null;
  private bitsPerValue: number;
   ...
   */
  private estimateBetaParameters(data: Float32Array): BetaParameters {
   ...
   */
  private generateBetaLevels(alpha: number, beta: number, numLevels: number): number[] {
    const levels: number[] = [];
   ...
   */
  private betaCF(x: number, alpha: number, beta: number): number {
    const maxIterations = 200;
    const epsilon = 1e-10;
  ...
  train(data: Float32Array): Codebook {
    ...
    const levels = this.generateBetaLevels(
      this.params.alpha,
      this.params.beta,
      numLevels
    );
    ...
    const entries: CodebookEntry[] = levels.map((level, i) => ({
      index: i,
      value: level,
...
function checkedAt(arr: readonly number[], i: number, label: string): number {
...
function assertCodebookShape(cb: TurboQuantCodebook): void {
  ...
  for (let i = 1; i < cb.boundaries.length; i++) {
    invariant(
      checkedAt(cb.boundaries, i, 'boundaries') > checkedAt(cb.boundaries, i - 1, 'boundaries'),
      `boundaries not strictly increasing at ${i}`
    );
  }
}
...
export class TurboQuantCodebookQuantizer {
  private readonly centroids: readonly number[];
  ...
  quantizeIndex(value: number): number {
    if (value <= checkedAt(this.boundaries, 0, 'boundaries')) return 0;
    if (value >= checkedAt(this.boundaries, this.boundaries.length - 1, 'boundaries')) return this.nClusters - 1;
 ...
 */
export function computeCodebookMse(codebook: TurboQuantCodebook): number {
  assertCodebookShape(codebook);
  let mse = 0;
  ...
  for (let i = 0; i < nClusters; i++) {
    ...
    const integrand = (x: number) => (x - c) ** 2 * sphereCoordinateBetaPdf(x, codebook.dimension);
...
export class TurboQuantBetaCodebook {
  private codebook: TurboQuantCodebook | null = null;
  private quantizer: TurboQuantCodebookQuantizer | null = null;

  constructor(
    private readonly dimension: number,
    private readonly bits: 1 | 2 | 3 | 4 | 5 | 6
  ) {}

  compute(options?: { maxIter?: number; tol?: number }): TurboQuantCodebook {
  ...
  getQuantizer(): TurboQuantCodebookQuantizer {
    if (!this.quantizer) throw new Error('Codebook not computed. Call compute() first.');
