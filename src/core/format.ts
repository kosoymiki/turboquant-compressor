...
export function encodeCompressedDatabase(
  vectors: Uint8Array[],
  dimensions: number,
  bitsPerValue: number,
  rotationSeed: number,
...
export function decodeCompressedDatabase(base64: string): CompressedDatabase {
...
function decodeV2(binary: Uint8Array, binaryView: DataView): CompressedDatabase {
  const HEADER_SIZE = 72;
...
function decodeV3(binary: Uint8Array, binaryView: DataView): CompressedDatabase {
  const dimensions = binaryView.getUint32(8, true);
  const paddedDimensions = binaryView.getUint32(12, true);
  const vectorCount = binaryView.getUint32(16, true);
  const bitsPerValue = binaryView.getUint8(20);
